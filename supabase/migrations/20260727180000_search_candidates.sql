-- Proximity search for the public candidate listing.
--
-- SECURITY INVOKER on purpose: RLS on `profiles` still applies, so the function
-- can only ever return profiles the caller is allowed to see — which means
-- `is_validated = true` for the public. Moderation is therefore enforced by the
-- database, not by the page that happens to call this.
--
-- The origin defaults to the caller's own profile location, which is the whole
-- point of asking for GPS at signup: an employer sees the candidates nearest to
-- their home. Explicit coordinates can be passed for visitors who share their
-- position without being registered.
