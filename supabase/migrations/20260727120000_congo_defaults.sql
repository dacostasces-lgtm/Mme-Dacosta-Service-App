-- The MVP was built assuming Côte d'Ivoire. The actual target market is the two
-- Congos, Congo-Brazzaville first, so the two places where a country is invented
-- on the user's behalf have to change: the `cities.country` column default and
-- the fallback inside the signup trigger.
--
-- No data backfill here on purpose. `cities` was empty when this ran, and
-- rewriting an existing 'Côte d'Ivoire' row to Congo would corrupt a legitimate
-- value rather than fix one.

ALTER TABLE public.cities ALTER COLUMN country SET DEFAULT 'Congo-Brazzaville';
...
BEGIN
  ...
  IF v_city IS NOT NULL THEN
    ...
    IF v_city_id IS NULL THEN
      ...
      VALUES (v_city, COALESCE(v_country, 'Congo-Brazzaville'))
